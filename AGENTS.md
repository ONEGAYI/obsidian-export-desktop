...
已知限制（审查登记，后续迭代评估；26.9 开发期已关闭五项：预扫描 comments 感知 #28、`.render-*` 惰性清扫与同文件嵌入全文回退 #27、测试 release 门 const 断言与引用正则兜底 #26）：

- [ ] 注释转换与 Obsidian 渲染的刻意差异（Obsidian 自身各视图间行为本就不一致，实现取自洽保守语义）：嵌套 `%%` 按非贪心配对（Obsidian 阅读视图会渲染错乱）；注释内嵌代码块被折叠进注释（Obsidian 阅读视图显示内容仅隐藏标记）；注释内容合成的容器字面是近似（列表 bullet 恒为 `-`、代码块 fence 恒为三反引号，仅影响注释内可读性）；跨块级边界的注释会把列表等结构在注释处断开再重启（CommonMark HTML 块无法内嵌于列表项中间，结构性必然）。前两条与最后一条的结构性部分保持现状；**有序列表重启回到起始编号**一条的旧登记理由（「事件模型无法表达接续」）已被推翻——序列化器尊重 `List(Some(n))` 起始编号，可在注释重开点合成接续编号，可修但暂缓（约 1 天、嵌套深度处理中风险）。
...
- [ ] cmd.exe 包装对工具路径含成对 `%` 的形态会做变量展开（引号不保护）：实测触发条件比登记更窄——须成对 `%` 恰好命中**已定义**的环境变量名（未定义名 cmd 命令行上下文字面保留），且 `%%` 加倍转义经实测不可靠（对已定义名折叠结果诡异、对未定义名不折叠），转义路线已证伪。罕见路径建议 `--diagram-bin` 指向 `.exe` 绕过包装（usage-advanced.md 已注明）；可选加固：resolve 时对含 `%` 的 `.cmd` 发 warning（约 15 行，暂缓）。
