...
* 「关于与更新」页：启动时（至多每天一次，可关闭）与按需检查 GitHub releases，展示 release notes，并可直接下载并启动新安装包。可选配置 HTTP 代理（主机 + 端口，跨会话记忆）：检测优先直连、失败自动经代理重试一次（共享代理出口的 GitHub 免费查询额度易被耗尽，能直连拿到就绝不消耗），下载则恒经代理。
