...
Fork 现状与上游有两点关键差异：**workflow 的 push/tag 事件从不触发**（Actions
runs 恒为 0，原因未查明），release workflow 必须手动 dispatch；**不等 runner
排队**（macOS/Linux 曾排队数小时），Windows 产物一律本地构建上传。
...
- [ ] `just set-version X.Y.Z` 一次对齐六处版本（根 crate 两处 + 桌面端四处）；
      CalVer `yy.m.i` 不得跳月，`just set-version` 拒绝降级
...
- [ ] push commit 与 tag 到 origin（fork）
- [ ] 手动 dispatch release workflow：`gh workflow run release.yml --ref vX.Y.Z
      -R ONEGAYI/obsidian-export-desktop`（dispatch 走 tag 所指 commit 上的
