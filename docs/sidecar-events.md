...
|------|------|------|
| `schema` | `version: number` | 同导出流，当前为 `1` |
| `update-result` | `outcome: string`, `channel: string`, `version?`, `htmlUrl?`, `notes?`, `assetName?`, `assetSize?` | 一次检测的结论。`outcome` 取 `available` / `up-to-date` / `no-release`，防御性还有 `unknown`——CLI 对无法归类的结论（如新版库引入的未知状态变体）显式降级为该值，退出码仍为 `0`，不当失败处理；消费端对未知新值也应降级。`channel` 为应答通道（`direct` / `proxied`，见下方 `--proxy` 语义；旧版 CLI 不带该字段，消费端按 null 容忍）。仅 `available` 时携带其余字段，`notes` 为 release 正文（可为 null），`assetName`/`assetSize` 为按 `--asset` 意图挑中的资产（无匹配时为 null，端侧引导 `htmlUrl` 手动下载） |
...
- **`--proxy` 双通道**（可选，`host:port` 或 `http://host:port`）：
  检测**优先直连**，直连任何失败（网络错/限流/解析）时经代理重试一
  次，代理结果（无论成败）为最终结果；直连成功则绝不发出代理请求
  （匿名 GitHub API 限额按 IP 计，共享代理出口的免费额度易被耗尽，
  串行回退而非并行双发即为此）。配了代理时下载恒经代理。未传该参数
  时纯直连（CLI 不读任何代理环境变量）；非法取值为参数错误（exit
  2）。`update-result` 的 `channel` 字段透出实际应答通道，供端侧展示
  「经代理」标注。
...
- **文本模式**（不带 `--progress json`）：stdout 打印当前版本、最新
  版本、发布页 URL 与资产信息；经代理应答时先打印一行
  `Checked via proxy` 提示；下载进度在 stderr 单行 `\r` 刷新（TTY
  判定，非 TTY 降级为完成时一行）。
